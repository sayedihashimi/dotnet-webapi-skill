...
**Date and time values ΓÇö use `DateTimeOffset`:** When a DTO includes a date or
...
conversions, and serializes to ISO 8601 with offset information in JSON ΓÇö which
is what API consumers expect.

Reference: https://learn.microsoft.com/en-us/dotnet/api/system.datetimeoffset

**Enum properties ΓÇö serialize as strings by default:** Unless the user
...
**Response DTOs** ΓÇö use positional sealed records for concise, immutable output:
...
**Request DTOs** ΓÇö use sealed records with `init` properties so data annotations
work naturally:
...
**Minimal API return types ΓÇö prefer `TypedResults`:**
...
**Fallback ΓÇö `Results` factory:** If a handler has many conditional branches
...
// Minimal API example ΓÇö TypedResults with explicit return type (recommended)
...
This is all that is needed ΓÇö no additional packages required.
...
Always define an interface for every service ΓÇö this enables unit testing with
...
1. Run `dotnet build` ΓÇö confirm zero errors and zero warnings.
...
| `TypedResults` in ternary without explicit return type | `Ok<T>` and `NotFound` have no common base ΓÇö annotate with `Task<Results<Ok<T>, NotFound>>` or fall back to `Results` factory. |
...
| Using `DateTime` for date/time properties | Use `DateTimeOffset` instead ΓÇö it preserves UTC offset, avoids timezone ambiguity, and serializes correctly in JSON. |
...
- [ASP.NET Core Web API overview](https://learn.microsoft.com/en-us/aspnet/core/web-api/) ΓÇö fundamental concepts for building Web APIs
- [OpenAPI in ASP.NET Core](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/openapi/overview) ΓÇö built-in OpenAPI support in .NET 9+
- [OpenAPI from XML comments](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/openapi/openapi-comments) ΓÇö how XML doc comments flow into the OpenAPI spec
- [Minimal APIs overview](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/minimal-apis/overview) ΓÇö routing, parameter binding, and response types
- [Handle errors in ASP.NET Core APIs](https://learn.microsoft.com/en-us/aspnet/core/web-api/handle-errors) ΓÇö Problem Details and exception handling
- [DateTimeOffset](https://learn.microsoft.com/en-us/dotnet/api/system.datetimeoffset) ΓÇö preferred type for date/time values in APIs
